...
def load_source_dataframe(retry_missing: bool = False) -> pd.DataFrame:
    ...
    if retry_missing:
        df_missing = pd.read_pickle("./2026_2/data/missing_climate_links.pkl")
        missing_ids = set(df_missing["city_id"])
        df_numbeo = df_numbeo[df_numbeo["city_id"].isin(missing_ids)].copy()

    df_numbeo.sort_values("country", inplace=True)
    return df_numbeo

def scrape_all_cities_to_db(retry_missing: bool = False, limit: int | None = None) -> None:
    """
